...
from score_test_calculations import (
    ...
    build_global_relative_sortino_analysis,
    build_global_relative_sortino_by_horizon,
    build_global_relative_sortino_observations,
...
ENABLED_TESTS = {
    ...
    "global_relative_sortino_optimization": True,
...
RELATIVE_SORTINO_PLOT_RANGES = [
    ("100-300", 100, 300, "100-300"),
    ("100-150", 100, 149, "100 <= T < 150"),
    ("150-200", 150, 199, "150 <= T < 200"),
    ("200-250", 200, 249, "200 <= T < 250"),
    ("250-300", 250, 300, "250 <= T <= 300"),
]
...
def annualize_return(total_return, horizon_days):
    ...
    return round(float((1 + total_return) ** (252 / horizon_days) - 1), 6)
...
def clean_plot_outputs(output_dir):
    for plot_dir in [
        ...
        for plot_file in plot_dir.rglob("*.png"):
            plot_file.unlink()
...
def save_analysis_outputs(
    ...
    global_relative_sortino_analysis,
    global_relative_sortino_by_horizon,
    global_relative_sortino_observations,
    ...
    global_relative_sortino_path = output_dir / "global_relative_sortino_analysis.csv"
    global_relative_sortino_by_horizon_path = (
        output_dir / "global_relative_sortino_by_horizon.csv"
    )
    global_relative_sortino_observations_path = (
        output_dir / "global_relative_sortino_observations.csv"
    )
...
def run_configured_score_tests(return_panel):
    ...
    global_relative_sortino_analysis = pd.DataFrame()
    global_relative_sortino_plot_analyses = {}
    global_relative_sortino_by_horizon = pd.DataFrame()
    global_relative_sortino_observations = pd.DataFrame()
    ...
    if ENABLED_TESTS["global_relative_sortino_optimization"]:
        global_relative_sortino_analysis = build_global_relative_sortino_analysis(
            return_panel
        )
        global_relative_sortino_plot_analyses = {
            folder_name: {
                ...
                "analysis": (
                    global_relative_sortino_analysis
                    if folder_name == "100-300"
                    else build_global_relative_sortino_analysis(
                        return_panel,
            ...
            for folder_name, horizon_start, horizon_end, label
            in RELATIVE_SORTINO_PLOT_RANGES
        }
        global_relative_sortino_by_horizon = (
            build_global_relative_sortino_by_horizon(return_panel)
        )
        global_relative_sortino_observations = (
            build_global_relative_sortino_observations(return_panel)
        )
    ...
    return {
        ...
        "global_relative_sortino_analysis": global_relative_sortino_analysis,
        "global_relative_sortino_plot_analyses": (
            global_relative_sortino_plot_analyses
        ),
        "global_relative_sortino_by_horizon": global_relative_sortino_by_horizon,
        "global_relative_sortino_observations": global_relative_sortino_observations,
...
def main():
    ...
    global_relative_sortino_analysis = score_tests["global_relative_sortino_analysis"]
    global_relative_sortino_plot_analyses = score_tests[
        "global_relative_sortino_plot_analyses"
    ]
    global_relative_sortino_by_horizon = score_tests[
        "global_relative_sortino_by_horizon"
    ]
    global_relative_sortino_observations = score_tests[
        "global_relative_sortino_observations"
    ]
